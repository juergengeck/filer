...
The expanded run observed a **5.093-second** added-photo receiver wait, versus 11.294 seconds in the earlier packaged run. Initial sharing still took 27.680 seconds to reach the receiver; startup latency remains a limitation. Concurrent workspace builds and older QA instances affected this session, so these are observed timings, not a controlled benchmark. Obsolete owned QA browsers were closed; the final Alice and unauthorized Charlie remain in separate origins in the inspection browser.
