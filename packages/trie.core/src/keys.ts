...
import type {Hash, KeyFn} from './types.js';
 ...
 */
export function hashPrefixKeyFn(chunkSize: number, maxDepth: number): KeyFn {
    return (entryHash: Hash) => {
        const chunks: string[] = [];
        const key = entryHash as string;
...
const DEPTH_LEVELS: Record<TimeDepth, number> = {
    day: 3,
    hour: 4,
    minute: 5
};

function dateToSegments(date: Date, trieId: string): string[] {
 ...
 */
export function timePathKeyFn(depth: TimeDepth, trieId: string): KeyFn {
    const levels = DEPTH_LEVELS[depth];
    return (_entryHash: Hash, context: Record<string, unknown>) => {
 ...
 */
export function timePathLeafKeys(from: Date, to: Date, trieId: string, depth: TimeDepth): string[][] {
    const levels = DEPTH_LEVELS[depth];
