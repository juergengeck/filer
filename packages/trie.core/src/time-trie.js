...
function dateToKeys(date, keyPrefix) {
...
function leafKeysInRange(from, to, keyPrefix, depth) {
    const keys = [];
...
export class TimeTrie {
    ...
    constructor(config, deps) {
        ...
        if (this.structuralTypeNames.length !== structuralNodeCount) {
            throw new Error(`TimeTrie(${config.trieId}): expected ${structuralNodeCount} structuralTypeNames, got ${this.structuralTypeNames.length}`);
        }
        if (this.structuralChildFields.length !== structuralNodeCount) {
            throw new Error(`TimeTrie(${config.trieId}): expected ${structuralNodeCount} structuralChildFields, got ${this.structuralChildFields.length}`);
        }
    }
    /**
     ...
     */
    async insert(entryHash, timestamp) {
        ...
        const updatedChildren = new Set(root[this.config.rootChildField]);
    ...
    async queryRange(from, to) {
        ...
        const results = [];
        const leaves = await Promise.all(leafKeys.map(async (key) => {
            try {
        ...
        for (const leaf of leaves) {
            if (leaf && !leaf.tombstoned) {
                const entries = leaf[this.config.leafEntryField];
    ...
    async queryLeaf(date) {
        ...
        try {
            ...
            return Array.from(leaf[this.config.leafEntryField]);
        }
        catch {
            return [];
        }
     ...
     * given timestamp.
     */
    async hasEntry(entryHash, timestamp) {
        const keys = dateToKeys(timestamp, this.keyPrefix);
        const leafKey = keys[this.structuralLevels - 1];
        try {
            const leaf = await this.loadLeafByKey(leafKey);
            ...
            return leaf[this.config.leafEntryField].has(entryHash);
        }
        catch {
            return false;
        }
     ...
     */
    async collectAllEntriesNewestFirst() {
        try {
            const { obj: root } = await this.getOrCreateRoot();
            const entries = [];
            const yearIdHashes = await this.sortNodeIdsNewestFirst(Array.from(root[this.config.rootChildField]));
     ...
     */
    async collectAllEntries() {
        try {
            const { obj: root } = await this.getOrCreateRoot();
            const children = root[this.config.rootChildField];
     ...
     */
    async reclaimStorage(maxUnitsToKeep) {
        ...
        try {
            const { obj: root } = await this.getOrCreateRoot();
            const yearIdHashes = root[this.config.rootChildField];
            for (const yearIdHash of yearIdHashes) {
     ...
     */
    async reclaimFromNode(idHash, level, cutoff) {
        let reclaimed = 0;
        try {
            ...
            if (level === this.structuralLevels - 2) {
                ...
                for (const childIdHash of children) {
                    try {
                        ...
                        if (leaf.tombstoned)
                            continue;
                        const leafDate = this.parseLeafKey(leaf.key);
                        if (leafDate && leafDate < cutoff) {
                            reclaimed += await this.tombstoneLeaf(leaf.key);
                        }
                    }
                    catch {
                        // Leaf not found
    ...
    async collectEntriesFromNode(idHash, level, entries) {
        try {
            const { obj: node } = await this.deps.getObjectByIdHash(idHash);
            if (level === this.structuralLevels - 1) {
                const leafEntries = node[this.config.leafEntryField];
    ...
    async collectEntriesNewestFirstFromNode(idHash, level, entries) {
        try {
            const { obj: node } = await this.deps.getObjectByIdHash(idHash);
            if (level === this.structuralLevels - 1) {
                if (node.tombstoned) {
            ...
            const childField = this.structuralChildFields[level];
            const childIdHashes = await this.sortNodeIdsNewestFirst(Array.from(node[childField]));
     ...
     */
    async getOrCreateStructuralNode(typeName, key, childField, childIdHash) {
        const skeleton = { $type$: typeName, key };
        const idHash = await this.deps.calculateIdHashOfObj(skeleton);
        try {
            const { obj } = await this.deps.getObjectByIdHash(idHash);
            const updatedChildren = new Set(obj[childField]);
            if (updatedChildren.has(childIdHash)) {
                return idHash;
            }
            updatedChildren.add(childIdHash);
     ...
     */
    async getOrCreateLeaf(key, entryHash) {
        const skeleton = { $type$: this.config.leafTypeName, key };
        const idHash = await this.deps.calculateIdHashOfObj(skeleton);
        try {
            const { obj } = await this.deps.getObjectByIdHash(idHash);
            const updatedEntries = new Set(obj[this.config.leafEntryField]);
     ...
     */
    async loadLeafByKey(key) {
        const skeleton = { $type$: this.config.leafTypeName, key };
        const idHash = await this.deps.calculateIdHashOfObj(skeleton);
        const { obj } = await this.deps.getObjectByIdHash(idHash);
        return obj;
    }
    async sortNodeIdsNewestFirst(idHashes) {
        const nodes = await Promise.all(idHashes.map(async (idHash) => {
            const { obj } = await this.deps.getObjectByIdHash(idHash);
